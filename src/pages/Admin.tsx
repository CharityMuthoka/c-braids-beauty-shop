...
const Admin = () => {
  ...
  const addProduct = useMutation({
    ...
    onSuccess: () => {
      ...
      setProductForm({ name: "", price: "", category: "perfume", image_url: "", description: "", stock: "0", featured: false });
    },
        ...
        <Tabs defaultValue="orders">
          ...
          <TabsContent value="products">
            <Card className="mb-6">
              ...
              <CardContent>
                <form onSubmit={(e) => { e.preventDefault(); addProduct.mutate(); }} className="grid md:grid-cols-2 gap-4">
                  ...
                  <Button type="submit" className="md:col-span-2 bg-gradient-rose"><Plus className="h-4 w-4 mr-2" />Add Product</Button>
                </form>
              </CardContent>
            </Card>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {products?.map((p) => (
                <Card key={p.id}><CardContent className="pt-4"><p className="font-semibold">{p.name}</p><p className="text-primary">${Number(p.price).toFixed(2)}</p><Badge variant="outline">{p.category}</Badge></CardContent></Card>
              ))}
            </div>
          </TabsContent>
        </Tabs>
